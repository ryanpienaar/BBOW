...
canopy_trees <- df |> filter(Height >= 20) # consider changing this
...
NPS <- NPS |> select(Accepted_LatinName, Diameter_cm, Status, Plot, genus, common_name, Northing, Easting, Transect )
...
Combo <- Combo |> filter(!Species %in% c("Ash", "Balsam", "Basswood", "Beech"))
...
Combo |> pivot_longer(cols = Aspen:Hemlock, names_to = "Species", values_to = "Cover") %>%
  mutate(
    Category = case_when(
      Cover == 1 ~ "Pure stands",
